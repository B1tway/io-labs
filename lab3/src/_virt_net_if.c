...
static char check_frame(struct sk_buff *skb) {
	...
	if ( (ip->version == IPVERSION) && (ip->daddr == nr_destipv4)) {
        ...
        char saddr_str[128];
        int len = sprintf(saddr_str, "saddr: %d.%d.%d.%d\n",
                saddr >> 24,
                (saddr >> 16) & 0x00FF,
                (saddr >> 8) & 0x0000FF,
                (saddr) & 0x000000FF);
        ...
        printk("Captured IPv4 packet, saddr: %d.%d.%d.%d; daddr: %d.%d.%d.%d\n",
                saddr >> 24, (saddr >> 16) & 0x00FF,
                (saddr >> 8) & 0x0000FF, (saddr) & 0x000000FF,
                daddr >> 24, (daddr >> 16) & 0x00FF,
                (daddr >> 8) & 0x0000FF, (daddr) & 0x000000FF);
